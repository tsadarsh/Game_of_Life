...
void populateNextGen(std::vector<std::vector<int>>* curr_gen, std::vector<std::vector<int>>* next_gen, std::vector<int> job)
{
    int i_row = job[0];
    int i_col = job[1];
...
void updateCurrGen(std::vector<std::vector<int>>* curr_gen, std::vector<std::vector<int>>* next_gen, std::vector<int> job)
{
    int i_row = job[0];
    int i_col = job[1];
...
void THRD_Process(std::vector<std::vector<int>>* curr_gen, std::vector<std::vector<int>>* next_gen, int num_of_threads, std::vector<std::vector<std::vector<int>>>* quotas)
{
    ...
    for (int iRep = 0; iRep < (*quotas).size(); iRep++)
    {
        for (int iThread = 0; iThread < num_of_threads; iThread++)
        {
            // std::vector<std::vector<int>>> q = &((*quotas)[iThread]);
            thread_pool_1.push_back(std::thread(populateNextGen, std::ref(curr_gen), std::ref(next_gen), (*quotas)[iRep][iThread]));
        }
    ...
    for (int iRep = 0; iRep < (*quotas).size(); iRep++)
    {
        for (int iThread = 0; iThread < num_of_threads; iThread++)
        {
            thread_pool_2.push_back(std::thread(updateCurrGen, std::ref(curr_gen), std::ref(next_gen), (*quotas)[iRep][iThread]));
        }
